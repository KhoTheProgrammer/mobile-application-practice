...
class DonationRepository {
    ...
    suspend fun createDonation(
        ...
        return try {
            val donationData = CreateDonationDto(
                donor_id = donorId,
                orphanage_id = orphanageId,
                category_id = categoryId,
                amount = amount,
                donation_type = donationType.name.lowercase(),
                status = "pending",
                ...
                quantity = quantity,
                note = note,
                recurring_frequency = recurringFrequency?.name?.lowercase(),
                need_id = needId
            )
    ...
    suspend fun updateDonationStatus(
        ...
        return try {
            val updates = UpdateDonationStatusDto(
                status = status.name.lowercase(),
                completed_at = if (status == DonationStatus.COMPLETED) "now()" else null
            )
