...
class UpdateNeedsViewModel(
    ...
    fun createNeed() {
        Log.d("UpdateNeedsViewModel", "createNeed called")
        if (!validateForm()) {
            Log.d("UpdateNeedsViewModel", "Form validation failed")
            return
        }

        viewModelScope.launch {
            uiState = uiState.copy(isLoading = true, error = null)
            Log.d("UpdateNeedsViewModel", "Creating need with orphanageId: $orphanageId")
            ...
            when (val result = needsRepository.createNeed(
                ...
                is NeedsResult.Success -> {
                    Log.d("UpdateNeedsViewModel", "Need created successfully")
                    uiState = uiState.copy(
                        isLoading = false,
                        isAddingNeed = false,
                ...
                is NeedsResult.Error -> {
                    Log.e("UpdateNeedsViewModel", "Failed to create need: ${result.message}")
                    uiState = uiState.copy(
                        isLoading = false,
                        error = result.message
                    )
